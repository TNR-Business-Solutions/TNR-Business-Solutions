...
/**
 * TNR Business Solutions - Final Wix Update
 * Updates your existing Import 1 and Import 2 collections
 */

const { createClient, OAuthStrategy } = require('@wix/sdk');
const { items } = require('@wix/data');

class FinalWixUpdater {
  constructor() {
  ...
  async insertToCollection(collectionName, content) {
    try {
  ...
  async updateHomepageContent() {
    try {
      ...
      // Try Import 1 first
      const import1 = 'Import 1';
      let success = await this.insertToCollection(import1, homepageData);
      if (!success) {
        // Try Import 2 if Import 1 fails
    ...
    try {
      ...
      for (const service of services) {
        try {
          ...
          // Try Import 1 first
          const import1 = 'Import 1';
          let success = await this.insertToCollection(import1, serviceData);
          if (!success) {
            // Try Import 2 if Import 1 fails
            const import2 = 'Import 2';
  ...
  async updateAllContent() {
    try {
      this.log('🚀 TNR Business Solutions - Final Wix Update', 'update');
      this.log('==========================================', 'update');
      ...
      if (!connectionOk) {
        this.log('❌ Cannot connect to Wix API', 'error');
        return false;
      }
      ...
      if (homepageSuccess || servicesSuccess) {
        ...
        this.log('3. Look for "Import 1" or "Import 2" collection', 'info');
...
module.exports = { FinalWixUpdater };
